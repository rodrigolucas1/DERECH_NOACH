    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      {categories?.length ? (
        <SlideUp className="mb-8 flex flex-wrap gap-2">
          {categories.map((cat) => (
            <span key={cat.id} className="flex items-center gap-1 rounded-full px-3 py-1 text-sm font-medium text-white transition-transform hover:scale-105" style={{ backgroundColor: cat.color ?? "#6b7280" }}>
              {cat.name}
            </span>
          ))}
        </SlideUp>
