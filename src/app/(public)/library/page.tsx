      ...
      <SlideUp className="mb-6">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full rounded-lg border bg-white py-2 pl-10 pr-4 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
          />
        </div>
      </SlideUp>

      {categories?.length ? (
        <SlideUp className="mb-6 flex flex-wrap gap-2">
          {categories.map((cat) => (
            <span key={cat.id} className="rounded-full bg-purple-50 px-3 py-1 text-sm font-medium text-purple-700 transition-transform hover:scale-105">
          ...
          {data.items.map((item) => {
            ...
            return (
              <StaggerItem key={item.id}>
                <AnimatedCard className="group rounded-lg border bg-white p-5 shadow-sm h-full">
                  <div className="flex items-start gap-3">
                    <div className="rounded-lg bg-gray-50 p-2 group-hover:bg-purple-50 transition-colors">
                      <Icon className="h-5 w-5 text-gray-600 group-hover:text-purple-600" />
                    </div>
                    <div className="flex-1">
                      <h3 className="font-medium text-gray-900 group-hover:text-purple-600 transition-colors">{item.title}</h3>
