        ...
        <StaggerContainer className="space-y-4">
          {data.items.map((e) => (
            <StaggerItem key={e.id}>
              <AnimatedCard className="flex items-start gap-4 rounded-lg border bg-white p-6 shadow-sm">
                <Link href={`/events/${e.id}`} className="flex items-start gap-4 flex-1">
                  <motion.div
                    whileHover={{ scale: 1.1 }}
                    className="flex h-16 w-16 flex-col items-center justify-center rounded-lg bg-blue-50 text-blue-700 shrink-0"
                  >
